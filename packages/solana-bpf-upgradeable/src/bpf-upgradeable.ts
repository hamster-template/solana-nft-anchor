...
export class BpfLoaderUpgradeable {
  ...
  static async loadBuffer(
    ...
    while (1) {
      ...
      // const bufferAccount = await Common.tryUntilSuccess(
      //   () => conn.getAccountInfo(bufferPk),
      //   500
      // );

      await Common.sleep(1000 * 2);
      const bufferAccount = await conn.getAccountInfo(bufferPk)

      const onChainProgramData = bufferAccount!.data.slice( // 切片
